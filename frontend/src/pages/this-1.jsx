//this file works with wav files
...
export default function AudiometryTest() {
  ...
  // NEW: State to track if audio files are loaded
  ...
  // NEW: Ref to cache the loaded audio files (AudioBuffers)
  ...
  // NEW: useEffect to preload all WAV files on component mount
  useEffect(() => {
  ...
  // --- Web Audio API tone player (REWRITTEN for WAV files) ---
  ...
  const playCurrentTone = () => {
    // Prevent playing if files are still loading
    ...
    // This timeout is a safety net for UI state, actual audio duration is from the file
    setTimeout(() => {
      setIsPlaying(false);
      acceptingGesturesRef.current = false;
    }, 1500); // Increased slightly for safety
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col items-center mb-6">
          ...
          {!camRunning && (
            <button
              onClick={startCamera}
              className="bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-lg mt-3"
              disabled={isLoading} // Disable until files are loaded
            >
              {isLoading
                ? "🔊 Loading Audio..."
        ...
        {/* Display loading indicator or test UI */}
        {isLoading ? (
