...
export default function FloatingChatbot() {
  ...
  const [position, setPosition] = useState({ x: 32, y: 32 }); // For `right: 32px`, `bottom: 32px`
  const [isDragging, setIsDragging] = useState(false);

  // === REFS ===
  const chatWindowRef = useRef(null);
  const dragStartRef = useRef(null); // Stores { mouseX, mouseY, windowX, windowY }
  ...
  // Effect to handle dragging logic (adding/removing document listeners)
  useEffect(() => {
    const handleDragMove = (e) => {
      ...
      // New position is relative to drag start
      // dx > 0 means mouse moved right, so `right` (position.x) should decrease
      // dy > 0 means mouse moved down, so `bottom` (position.y) should decrease
      setPosition({
        x: dragStartRef.current.windowX - dx,
        y: dragStartRef.current.windowY - dy,
      });
    };
  ...
  // --- 1. Render Floating Button (if closed) ---
  if (!isOpen) {
    return (
      <button
  ...
  // --- 2. Render Chat Window & Modals (if open) ---
  return (
    <>
      {/* --- Summary Modal --- */}
      {showSummary && (
      ...
      {/* --- Chat Window --- */}
      <div
        className="fixed w-96 h-[600px] bg-white rounded-lg shadow-2xl flex flex-col font-sans transition-all z-40"
        // ✨ Apply dynamic position from state
        style={{
      ...
      >
        ...
        <div
          ...
          // ✨ Add drag handlers and cursor style
        ...
        >
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="p-1 rounded-full hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-white disabled:opacity-50"
            ...
            >
              ...
              >
                <path
                  fillRule="evenodd"
                  d="M9 2a1 1 0 00-1 1v1H4a1 1 0 000 2h1v9a2 2 0 002 2h6a2 2 0 002-2V6h1a1 1 0 100-2h-4V3a1 1 0 00-1-1H9zm-1 4v9a1 1 0 001 1h4a1 1 0 001-1V6H8z"
                  clipRule="evenodd"
                />
              </svg>
            </button>
        ...
        >
          ...
          <button
            ...
            className="bg-green-600 text-white font-semibold rounded-lg px-5 py-3 shadow-md hover:bg-green-700 transition-all disabled:opacity-50"
